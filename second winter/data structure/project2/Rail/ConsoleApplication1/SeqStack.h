...
public:
    ...
    void Clear();									// 将栈清空
    void Traverse(const std::function<void(const ElemType&)>& Visit) const;	// 遍历栈
...
template <class ElemType>
void SeqStack<ElemType>::Traverse(const std::function<void(const ElemType&)>& Visit) const {
    for (int i = top; i >= 0; i--) {
        Visit(elems[i]);
    }
}
